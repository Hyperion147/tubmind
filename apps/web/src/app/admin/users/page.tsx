...
export default async function AdminUsersPage({ searchParams }: PageProps) {
    ...
    const userFilters = [
        ...
        ...(userStatusFilter !== "all"
            ? [eq(profiles.status, userStatusFilter)]
            : []),
    ];
    ...
    const [userStats, ideaCounts, commentCounts, recentLogs] =
        await Promise.all([
            getAdminUserStats(),
            shownUserIds.length
                ? db
            ...
            db
                ...
                .innerJoin(
                    profiles,
                    eq(moderationLogs.targetUserId, profiles.id),
                )
    ...
    const ideaCountMap = new Map(
        ideaCounts.map((row) => [row.ownerId, row.value]),
    );
    const commentCountMap = new Map(
        commentCounts.map((row) => [row.authorId, row.value]),
    );
