...
export function LandingAppAccessCta() {
    ...
    return (
        <section
            id="app"
            className="relative mt-20 scroll-mt-28 overflow-hidden border border-border bg-[color-mix(in_oklch,var(--accent)_18%,var(--background))] px-4 text-left sm:px-6 md:scroll-mt-32 md:px-8 lg:px-12"
        >
            <div
                aria-hidden="true"
            ...
            <div className="grid items-center gap-6 py-4 md:py-6 lg:grid-cols-[1.25fr_0.9fr]">
                <DashboardPreview />

                <div className="relative px-1 py-2 sm:px-2 sm:py-3 md:px-5">
                    <motion.svg
                        aria-hidden="true"
                    ...
                    <h2 className="mt-3 max-w-md text-2xl font-semibold leading-tight tracking-tight text-foreground sm:text-[1.75rem] md:text-3xl">
                    ...
                    <div className="mt-5 flex flex-col items-start gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                        <Button
                            asChild
                            className="w-full shadow-[3px_3px_0_0_var(--color-border)] sm:w-auto"
                        >
...
function DashboardPreview() {
    return (
        <div
            className="pointer-events-none relative mx-auto w-[320px] max-w-full overflow-hidden border border-border bg-card/86 shadow-[12px_16px_36px_-28px_color-mix(in_oklch,var(--foreground)_42%,transparent)] transform-3d md:w-full md:max-w-none md:translate-y-12 md:-rotate-2 md:min-h-64"
        >
            ...
            <div className="relative grid min-h-52 grid-cols-[3rem_1fr] sm:grid-cols-[3.5rem_1fr] md:min-h-40">
                <aside className="border-r border-border bg-card/92 p-1.5 sm:p-2">
                    <div className="grid gap-2">
                        {dashboardNav.map((Icon, index) => (
                            <div
                                key={index}
                                className={cn(
                                    "flex size-7 items-center justify-center border border-border bg-background/70 text-muted-foreground sm:size-8",
                                    index === 0 &&
                ...
                <div className="p-3 sm:p-4">
                    ...
                    <div className="grid gap-2 sm:grid-cols-3">
                        {dashboardStats.map(([label, value, delta]) => (
                            <div
                                key={label}
                                className="border border-border bg-background/70 p-2.5 shadow-sm sm:p-3"
                            >
                    ...
                    <div className="mt-3 grid gap-3 md:grid-cols-[1fr_1.2fr]">
                        <div className="border border-border bg-background/70 p-2.5 sm:p-3">
                            <p className="text-[10px] text-muted-foreground">
                                Today&apos;s focus
                            </p>
